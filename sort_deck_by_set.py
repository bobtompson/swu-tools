...
# TS26 cards exist only in the four pre-constructed Twin Suns decks, never in a
# collectable set, so they can't be pulled from set-organized binders. This
# mapping (sourced from the community "TS Pre-Con Deck Breakdown" sheet and
# generated by build_ts26_decks.py) records which pre-con(s) each TS26 card came
# from, so the sorted output can point you at the right deck box.
TS26_DECKS_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), 'card_data', 'ts26_decks.json'
)
_ts26_decks = None


def get_ts26_decks(number):
    """Return the list of pre-con decks a TS26 card number belongs to."""
    global _ts26_decks
    if _ts26_decks is None:
        try:
            with open(TS26_DECKS_PATH, encoding='utf-8') as f:
                _ts26_decks = json.load(f).get('cards', {})
        except (OSError, ValueError):
            _ts26_decks = {}
    entry = _ts26_decks.get(str(number).zfill(3))
    return entry.get('decks', []) if entry else []
...
def format_output(grouped, metadata=None):
    ...
    for set_abbr in ordered_sets:
        ...
        for card in cards:
            ...
            if set_abbr == 'TS26':
                origin = get_ts26_decks(card['number'])
                if origin:
                    line += f" [from: {', '.join(origin)}]"
            lines.append(line)
